...
def test_robotics_sandbox_enabled():
    ...
    assert policy["actuator_limits"]["max_torque_Nm"] == 2.0, "Torque limit should be 2.0 Nm"
    assert policy["actuator_limits"]["max_speed_mps"] == 0.5, "Speed limit should be 0.5 m/s"
    ...
    assert set(policy["materials_whitelist"]) == expected_materials, "Materials whitelist should match expected"
    ...
    assert set(policy["banned_terms"]) == expected_banned, "Banned terms should match expected"
...
def test_sandbox_limits_configuration():
    ...
    assert "operating_area" in sandbox_limits, "Operating area limits should be configured"
    assert sandbox_limits["operating_area"]["max_radius_m"] == 50, "Max radius should be 50m"
    assert sandbox_limits["operating_area"]["require_geofence"] is True, "Geofence should be required"
    ...
    assert session_limits["max_duration_minutes"] == 30, "Max session duration should be 30 minutes"
    assert session_limits["max_operations_per_session"] == 10, "Max operations should be 10 per session"
    assert session_limits["cooldown_between_sessions_minutes"] == 15, "Cooldown should be 15 minutes"
    ...
    assert monitoring["video_recording_required"] is True, "Video recording should be required"
    assert monitoring["real_time_telemetry"] is True, "Real-time telemetry should be enabled"
    assert monitoring["alert_on_anomalies"] is True, "Anomaly alerts should be enabled"
    
    # Verify safety boundaries
    assert "safety_boundaries" in sandbox_limits, "Safety boundaries should be configured"
    safety = sandbox_limits["safety_boundaries"]
    assert safety["emergency_stop_required"] is True, "Emergency stop should be required"
    assert safety["human_supervisor_required"] is True, "Human supervisor should be required"
...
def test_environment_variable_override():
    ...
    policy = load_policy()
    assert policy["self_repair_enabled"] is True, "Environment variable should enable self-repair"
...
def test_config_file_fallback():
    ...
    try:
        ...
        assert policy["approve_required"] is True, "Should default to requiring approval"
        assert policy["self_repair_enabled"] is False, "Should default to no self-repair"
        assert "sandbox_limits" not in policy, "Should not have sandbox_limits in defaults"
