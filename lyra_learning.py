import os, json, re, smtplib, datetime as dt, pathlib, requests, yaml
...
ALLOW_TOPICS = ["health","medicine","nursing","hospital","public health","ai","machine learning","accessibility","assistive","security","privacy","ethics","education","productivity"]
DENY_TOPICS  = ["celebrity","gossip","rumor","leak","nsfw","sports scandal","casino","get rich quick"]
BLOCK_WORDS  = ["slur1","slur2","extreme phrase"]
SAFETY = {}; ROBOTICS_POLICY = {}
...
def load_memory():
    ...
    try: return json.load(open(MEM_PATH,"r",encoding="utf-8"))
    except: return []

def save_memory(mem):
    os.makedirs(os.path.dirname(MEM_PATH), exist_ok=True)
    json.dump(mem[-200:], open(MEM_PATH,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
...
def fetch_sources():
    ...
    for url in SEED_URLS:
        try:
            ...
            if text: chunks.append({"url":url, "text":text[:4000]})
        except Exception as e:
            chunks.append({"url":url, "error":str(e)})
    return chunks

def matches_any(text, needles): low=text.lower(); return any(n in low for n in needles)
def filter_chunks(chunks):
    kept, skipped = [], []
    for c in chunks:
        if "error" in c: skipped.append(c | {"reason": f"fetch error: {c['error']}" }); continue
        t=c["text"]
        if matches_any(t, DENY_TOPICS): skipped.append(c | {"reason":"deny-topic"}); continue
        if ALLOW_TOPICS and not matches_any(t, ALLOW_TOPICS): skipped.append(c | {"reason":"not in allow list"}); continue
        kept.append(c)
    return kept, skipped
...
# ==== Summarize + Glossary ====
JARGON_RE = re.compile(r"\b([A-Z][a-zA-Z0-9\-]{2,}(?:\s+[A-Z][a-zA-Z0-9\-]{2,}){0,2})\b")
def extract_terms(text, n=10):
    counts={}
    for m in JARGON_RE.findall(text):
        k=m.strip()
        if k.lower() in {"the","and","for","with","from","this","that"}: continue
...
def summarize_simple(chunks, prev_plan):
    ...
    return {"summary":"Fetched sources:\n"+"\n".join(lines),
            "insights":["Basic ingestion only (no model key set)."],
            "next_plan": (prev_plan[:3] if prev_plan else ["Deep-dive one allow-listed source"]) }
...
{corpus}"""
    resp = openai.ChatCompletion.create(
        model="gpt-4o-mini",
        messages=[{"role":"system","content":"Concise daily learner."},
    ...
    for line in out.splitlines():
        s=line.strip(" -•\t")
        if not s: continue
        if any(k in s.lower() for k in ["tomorrow","next","plan"]): next_plan.append(s)
        else: insights.append(s)
    return {"summary": out[:1200], "insights": insights[:8], "next_plan": (next_plan or ["Return to a medical/AI source"])[:5]}
...
{context[:6000]}"""
    resp = openai.ChatCompletion.create(
        model="gpt-4o-mini",
        messages=[{"role":"system","content":"Helpful glossary generator."},
                  {"role":"user","content":prompt}],
        temperature=0.2,
    )
...
def robotics_brainstorm(kept, policy, use_openai):
    ...
    proposal = {
        ...
        "safety_limits":{"max_speed_mps":max_speed,"max_joint_torque_Nm":max_torque,"materials_only":materials,"no_hazardous_functions":True,"human_approval_required":True},
        "modules":["2‑wheel base + caster","SBC + mic array","USB depth/RGB cam","Speakers + TTS","8–10'' tablet UI"],
        "printables":["chassis plates (PLA/PETG)","sensor mast (TPU coupler)","tablet bracket","cable guides"],
    ...
    if use_openai:
        try:
            ...
            openai.api_key = OPENAI_API_KEY
            system = (f"Safety-first robotics planner. Propose a single small, low-power, indoor assistive concept. "
            ...
            try: proposal = _json.loads(res.choices[0].message["content"])
            except: pass
        except: pass
...
def read_last_lines(path, n=20):
    try: return open(path,"r",encoding="utf-8").read().splitlines()[-n:]
    except: return []

def summarize_admin_mode_events():
    p = pathlib.Path("data/admin_mode.log")
    lines = read_last_lines(str(p), 20)
    if not lines: return "(no recent admin mode events)"
    out=[]
    for ln in lines:
        try:
            ev = json.loads(ln)
            ...
            if t=="admin_mode_update":
                ...
                out.append(f"- mode: {b.get('admin')}→{a.get('admin')}, persona: {b.get('personality')}→{a.get('personality')} (via {ev.get('used')})")
            elif t=="auth_failed":
...
def summarize_pitch_history():
    ...
    if not p.exists(): return "(no pitch decisions yet)"
    try:
        data=json.loads(p.read_text(encoding="utf-8")); hist=data.get("history", [])[-10:]
        if not hist: return "(no pitch decisions yet)"
        return "\n".join(f"- [{h.get('decision','?').upper()}] {h.get('title','(untitled)')} (id {h.get('id')})" for h in hist)
    except: return "(could not read pitch history)"

def make_email_body(entry, safety_audit:str, admin_summary:str, pitches_summary:str):
    parts = [
        f"👋 Lyra Daily Learning — {entry['date']}",
        "",
        "Summary:", safe_scrub(entry["summary"]),
        "", "Insights:",
    ...
    ] + [f"• {s['url']} — {s.get('reason','')}" for s in entry.get("sources_skipped", [])] + [
...
if __name__ == "__main__":
    ...
    if not kept: kept = [c for c in raw if "error" not in c][:2]

    if USE_OPENAI: analysis = summarize_with_openai(kept, memory)
    else:
    ...
    if terms and USE_OPENAI:
        gloss = [g.strip() for g in define_with_openai(terms, combined).splitlines() if g.strip()]
    elif terms:
        gloss = [f"{t}: (definition pending — no model key set)" for t in terms]  # fallback
    else:
        gloss = []

    # robotics idea (double gate)
    robotics_enabled = bool(ROBOTICS_POLICY.get("enabled")) and os.getenv("ROBOTICS_ENABLE","0").lower() in {"1","true","yes"}
    robotics_section = robotics_brainstorm(kept, ROBOTICS_POLICY, USE_OPENAI) if robotics_enabled else "(disabled)"

    # self-repair status (call robot_core if running)
    try:
        diag = requests.post(os.getenv("ROBOT_CORE_URL","http://localhost:8088")+"/robot/diagnose", timeout=3).json()
        plan = requests.post(os.getenv("ROBOT_CORE_URL","http://localhost:8088")+"/robot/repair/plan", timeout=5).json()
    ...
    except: safety_audit = "(audit log unavailable)"
