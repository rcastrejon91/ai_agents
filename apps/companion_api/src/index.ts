...
import { validateEnvironment, getEnvironmentConfig } from "./config/env-validation.js";
...
const corsOptions = {
  origin: function (origin: string | undefined, callback: (err: Error | null, allow?: boolean) => void) {
    ...
    const allowedOrigins = Array.isArray(appConfig.cors.origin) ? appConfig.cors.origin : [];
...
app.use((req, res, next) => {
  const ip = req.headers['x-forwarded-for'] as string || req.headers['x-real-ip'] as string || req.connection.remoteAddress || 'unknown';
  const now = Date.now();
  
  const current = rateLimit.get(ip);
  if (!current || now > current.resetTime) {
    rateLimit.set(ip, { count: 1, resetTime: now + appConfig.rateLimit.windowMs });
  } else {
    current.count++;
...
app.listen(PORT, () => {
  ...
  console.log(`⏱️  Rate limit: ${appConfig.rateLimit.maxRequests} requests per ${appConfig.rateLimit.windowMs / 1000 / 60} minutes`);
});
