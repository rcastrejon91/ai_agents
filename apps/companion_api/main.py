...
async def add_latency_and_request_id(request: Request, call_next):
    ...
    if process_time > 1000:
        logger.warning(
            f"[{req_id}] {request.method} {request.url.path} took {process_time:.2f}ms"
        )
    else:
        logger.info(
            f"[{req_id}] {request.method} {request.url.path} completed in {process_time:.2f}ms"
        )
...
async def handle(payload: InputPayload, request: Request):
    ...
    logger.info(
        f"[{req_id}] Incoming payload: text={payload.text}, metadata={payload.metadata}"
    )
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"request_id": req_id, "error": str(e)}
        )
