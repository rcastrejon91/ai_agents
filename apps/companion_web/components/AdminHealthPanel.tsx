        ...
        {robots.map((r) => (
          <div key={r.id} className="rounded border border-zinc-800 p-3">
            <div className="flex items-center justify-between">
              ...
              <div
                className={`text-xs ${r.status === "error" ? "text-rose-400" : "text-emerald-400"}`}
              >
                {r.status}
              </div>
            </div>
            <div className="text-sm text-zinc-400">
              Zone <b>{r.pose.zone}</b> • Pos ({r.pose.x.toFixed(1)},
              {r.pose.y.toFixed(1)})<br />
              Battery <b>{r.battery.toFixed(0)}%</b> • Temp{" "}
              <b>{r.health.tempC.toFixed(1)}°C</b>
            </div>
