...
export default function ActionRunner({ query }: { query: string }) {
  ...
  async function run() {
    ...
    for (const step of plan) {
      ...
      const argsFilled = Object.fromEntries(
        Object.entries(step.args || {}).map(([k, v]) => [
          k,
          String(v).replace(/\{\{([^}]+)\}\}/g, (_, p) => {
            const parts = p.split(".");
            let v: any = { plan: ctx.plan };
      ...
      try {
        res = tool ? await tool(argsFilled) : res;
      } catch (e: any) {
        res = { ok: false, text: e?.message || "tool error" };
      }
            ...
            {plan.map((s) => (
              <li key={s.id} className="border border-zinc-800 rounded-lg p-2">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="badge mr-2">#{s.id}</span>
                    <b>{s.tool}</b>{" "}
                    {s.note && (
                ...
                {out.find((o) => o.id === s.id) && (
                  <div className="mt-2 text-sm">
                    <div className="text-zinc-200">
                      {out.find((o) => o.id === s.id)?.title}
                    </div>
                    <div className="text-zinc-300 whitespace-pre-wrap">
                      {out.find((o) => o.id === s.id)?.text}
                    </div>
                    {out.find((o) => o.id === s.id)?.cite && (
                      <a
                        className="text-xs underline text-zinc-400"
                        href={out.find((o) => o.id === s.id)?.cite}
                        target="_blank"
                      >
                        source
                      </a>
