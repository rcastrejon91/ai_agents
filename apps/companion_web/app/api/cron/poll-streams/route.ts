...
async function latestToken(supa: any, provider: 'twitch'|'youtube') {
  const { data } = await supa.from('oauth_tokens')
    .select('*')
...
async function twitchLive(accessToken: string, userId: string, clientId: string) {
...
async function youtubeLive(accessToken: string) {
  const r = await fetch('https://www.googleapis.com/youtube/v3/liveBroadcasts?part=snippet,status&broadcastStatus=active&mine=true', {
...
export async function GET(_req: NextRequest) {
  ...
  if (tw) {
    const now = await twitchLive(tw.access_token, tw.provider_user_id, process.env.TWITCH_CLIENT_ID!);
    const { data: st } = await supa.from('stream_status').select('*')
      .eq('provider','twitch').eq('provider_user_id', tw.provider_user_id).maybeSingle();
    ...
    if (now.live !== wasLive) {
      ...
      if (now.live) {
        await discordPost(`🔴 **Twitch LIVE** — ${tw.provider_login} just went live: ${now.title ?? ''}\nhttps://twitch.tv/${tw.provider_login}`);
  ...
  if (yt) {
    ...
    const { data: st } = await supa.from('stream_status').select('*')
      .eq('provider','youtube').eq('provider_user_id', yt.provider_user_id).maybeSingle();
    ...
    if (now.live !== wasLive) {
      ...
      if (now.live) {
        await discordPost(`🔴 **YouTube LIVE** — ${yt.provider_login} just went live: ${now.title ?? ''}\nhttps://youtube.com/channel/${yt.provider_user_id}/live`);
