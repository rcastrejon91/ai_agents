...
async function topScores(supa:any, game='module-battle', days=1, limit=10) {
...
export async function GET(req: NextRequest) {
  ...
  if (!list.length) {
    await discordPost(`📊 ${period === 'weekly' ? 'Weekly' : 'Daily'} leaderboard for **${game}**: No scores yet.`);
  ...
  await discordPost(`📊 ${period === 'weekly' ? 'Weekly' : 'Daily'} leaderboard — **${game}**\n${lines}`);
