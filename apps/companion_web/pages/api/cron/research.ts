...
async function openPrWithPatch(
  ...
  const pr = await gh(`/repos/${repo}/pulls`, {
    ...
    body: JSON.stringify({
      ...
      body: "Automated proposal via Research Loop (human review required).",
    }),
  }).then((r) => r.json());
...
export default async function handler(
  ...
  try {
    ...
    const { data: task } = await sb
      ...
      .single();
    if (!task)
      return res.status(200).json({ ok: true, note: "no queued tasks" });
    ...
    if (!gate.allowed) {
      ...
      return res
        .status(200)
        .json({ ok: false, reason: "blocked by guardrails" });
    }
