...
if ENVIRONMENT == "production" and SECRET_KEY == "dev-key-change-in-production":
    ...
    print(
        "⚠️  WARNING: Generated random SECRET_KEY. Set SECRET_KEY env var for persistence."
    )
...
class LyraConfig:
    ...
    perspective_weights: Dict[str, float] = field(
    ...
    def validate(self) -> List[str]:
        ...
        if not (0.95 <= weight_sum <= 1.05):
            issues.append(
                f"Perspective weights sum to {weight_sum:.2f} (should be ~1.0)"
            )

        # Check autonomy level
        if not (0.0 <= self.autonomy_level <= 1.0):
            issues.append(
                f"Autonomy level {self.autonomy_level} out of range [0.0, 1.0]"
            )
...
def print_config_summary():
    ...
    print(
        f"  • Search (Serper/SerpAPI/Tavily): {'✅' if any([SERPER_API_KEY, SERPAPI_KEY, TAVILY_API_KEY]) else '❌'}"
    )
    ...
    agents = [
        ...
        (
            "💰 Finance",
            finance_agent_config.enabled,
            finance_agent_config.risk_tolerance,
        ),
        (
            "✍️  Content",
            content_agent_config.enabled,
            content_agent_config.default_tone,
        ),
        (
            "💻 Dev",
            dev_agent_config.enabled,
            f"{len(dev_agent_config.languages)} languages",
        ),
        (
            "🔬 Research",
            research_agent_config.enabled,
            research_agent_config.search_depth,
        ),
    ]
