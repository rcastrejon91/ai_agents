...
def register_error_handlers(app) -> None:
    ...
    def handle_unauthorized(error: HTTPException) -> Tuple[Dict[str, Any], int]:
        ...
        app.logger.warning(f'Unauthorized access attempt: {request.url} - IP: {request.environ.get("REMOTE_ADDR")}')
    ...
    def handle_forbidden(error: HTTPException) -> Tuple[Dict[str, Any], int]:
        ...
        app.logger.warning(f'Forbidden access attempt: {request.url} - IP: {request.environ.get("REMOTE_ADDR")}')
    ...
    def handle_rate_limit(error: HTTPException) -> Tuple[Dict[str, Any], int]:
        ...
        app.logger.warning(f'Rate limit exceeded: {request.url} - IP: {request.environ.get("REMOTE_ADDR")}')
    ...
    def handle_http_error(error: HTTPException) -> Tuple[Dict[str, Any], int]:
        ...
        app.logger.warning(f'HTTP error {error.code}: {error.description} - {request.url}')
...
def log_request_info(app) -> None:
    ...
    def log_request():
        ...
        if not app.debug:  # Only log in production to avoid noise
            app.logger.info(f'{request.method} {request.url} - IP: {request.environ.get("REMOTE_ADDR")}')
