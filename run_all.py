...
class ProcessManager:
    ...
    def check_processes(self):
        ...
        for name, process in self.processes.items():
            if process.poll() is None:
                print(
                    f"  {Color.OKGREEN}✅ {name}: Running (PID: {process.pid}){Color.ENDC}"
                )
            else:
...
def check_dependencies():
    ...
    required_packages = ["flask", "openai", "anthropic", "numpy", "pandas", "nltk"]
    ...
    if env_file.exists():
        ...
        print(
            f"  {Color.WARNING}⚠️  .env file not found (will use defaults){Color.ENDC}"
        )
...
def start_companion_api(pm: ProcessManager):
    ...
    return pm.start_process(
        "API", ["npm", "run", "dev"], cwd=api_path, color=Color.OKCYAN
    )
...
def start_companion_web(pm: ProcessManager):
    ...
    return pm.start_process(
        "WEB", ["npm", "run", "dev"], cwd=web_path, color=Color.HEADER
    )
...
def start_agent_gateway(pm: ProcessManager):
    ...
    return pm.start_process(
        "GATEWAY", [sys.executable, "api_gateway.py"], color=Color.OKGREEN
    )
...
def start_lyra_core(pm: ProcessManager):
    ...
    return pm.start_process(
        "LYRA", [sys.executable, str(lyra_file)], color=Color.WARNING
    )
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Lyra AI Platform Launcher")
    parser.add_argument(
        "--components",
        nargs="+",
    ...
    parser.add_argument(
        "--skip-check", action="store_true", help="Skip dependency check"
    )
