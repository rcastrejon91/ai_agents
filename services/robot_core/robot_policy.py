...
def validate_policy_config(policy):
    ...
    if "materials_whitelist" in policy and not isinstance(policy["materials_whitelist"], list):
    ...
    if policy.get("enabled", False) and not policy.get("approve_required", False):
        validation_errors.append("approve_required must be True when robotics is enabled")
...
def check_environment_safety():
    ...
    repair_enabled = os.getenv("ROBOTICS_REPAIR_ENABLE", "0").lower() in {"1", "true", "yes"}
    
    if robotics_enabled and repair_enabled:
        warnings.append("WARNING: Both ROBOTICS_ENABLE and ROBOTICS_REPAIR_ENABLE are set - this enables autonomous operation")
    
    if repair_enabled and not robotics_enabled:
        warnings.append("WARNING: ROBOTICS_REPAIR_ENABLE is set but ROBOTICS_ENABLE is not - self-repair requires robotics to be enabled")
...
def load_policy():
    ...
    if os.path.exists(TOPIC_CONFIG_FILE):
        try:
            ...
            logger.warning(f"Failed to load configuration from {TOPIC_CONFIG_FILE}: {str(e)}")
    ...
    if os.getenv("ROBOTICS_REPAIR_ENABLE", "0").lower() in {"1", "true", "yes"}:
        ...
        if p.get("enabled", False):
            logger.warning("Self-repair enabled via ROBOTICS_REPAIR_ENABLE environment variable - this enables autonomous operation")
            p["self_repair_enabled"] = True
        else:
            logger.error("ROBOTICS_REPAIR_ENABLE is set but robotics is not enabled - ignoring self-repair setting for safety")
