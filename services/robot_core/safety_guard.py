...
class PrimeDirective:
    ...
    def evaluate(self, action: str, context: Dict[str, Any]) -> Dict[str, Any]:
        if not self.cfg.get("prime_directive", True):
            return {"allow": False, "reason": "prime_directive_disabled"}
        if action in FORCEFUL or action in set(self.cfg.get("forbidden_actions", [])):
            _audit(f"DENY {action} forceful")
            return {"allow": False, "reason": "violent_action_forbidden"}
        if (
            self.cfg.get("allow_nonviolent_only", True)
            and action not in ALLOWED_PASSIVE
        ):
            _audit(f"DENY {action} not_in_passive_allowlist")
            return {"allow": False, "reason": "not_passive_allowlisted"}
        ...
        _audit(
            f"ALLOW {action} ctx={json.dumps({k:v for k,v in context.items() if k!='video'})[:200]}"
        )
