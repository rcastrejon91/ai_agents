...
class LyraOrchestrator:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.perspective_weights = self.config.get("perspective_weights", {
            "Pragmatist": 0.20,
            "Visionary": 0.15,
            "Analyst": 0.20,
            "Creator": 0.15,
    ...
    async def analyze_intent(self, user_input: str, context: Dict) -> Dict:
        ...
        if any(word in user_lower for word in ["research", "study", "analyze", "investigate", "explain"]):
            return {"type": "research", "complexity": "medium", "raw": user_input}
        
        elif any(word in user_lower for word in ["health", "medical", "biohacking", "supplement", "nad", "longevity"]):
            return {"type": "medical", "complexity": "medium", "raw": user_input}
        
        elif any(word in user_lower for word in ["money", "invest", "trade", "profit", "finance"]):
            return {"type": "finance", "complexity": "medium", "raw": user_input}
        
        elif any(word in user_lower for word in ["write", "create", "generate", "content", "blog"]):
            return {"type": "content", "complexity": "simple", "raw": user_input}
        
        elif any(word in user_lower for word in ["code", "build", "develop", "program", "app"]):
    ...
    async def execute_decision(self, decision: Dict, intent: Dict) -> Dict:
        ...
        return {
            "message": f"Lyra processed your request using multi-perspective analysis",
            "intent": intent['type'],
